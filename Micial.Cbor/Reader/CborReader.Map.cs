    ...
    public partial class CborReader
    {
        ...
        public int? ReadStartMap()
        {
            ...
            if (header.AdditionalInfo == CborAdditionalInfo.IndefiniteLength)
            {
                ...
                {
                    throw new CborContentException(MSR.Format(MSR.Cbor_ConformanceMode_RequiresDefiniteLengthItems, ConformanceMode));
                }
            ...
            else
            {
                ...
                {
                    throw new CborContentException(MSR.Cbor_Reader_DefiniteLengthExceedsBufferSize);
                }
        ...
        public void ReadEndMap()
        {
            if (_definiteLength is null)
            {
                ...
                if (_itemsRead % 2 != 0)
                {
                    throw new CborContentException(MSR.Cbor_Reader_InvalidCbor_KeyMissingValue);
                }
        ...
        private void ValidateSortedKeyEncoding((int Offset, int Length) currentKeyEncodingRange)
        {
            ...
            if (_previousKeyEncodingRange != null)
            {
                ...
                if (cmp > 0)
                {
                    ...
                    throw new CborContentException(MSR.Format(MSR.Cbor_ConformanceMode_KeysNotInSortedOrder, ConformanceMode));
                }
                else if (cmp == 0 && CborConformanceModeHelpers.RequiresUniqueKeys(ConformanceMode))
                {
                    ResetBuffer(currentKeyEncodingRange.Offset);
                    throw new CborContentException(MSR.Format(MSR.Cbor_ConformanceMode_ContainsDuplicateKeys, ConformanceMode));
                }
            }
