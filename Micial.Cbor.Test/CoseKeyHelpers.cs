    ...
    public static class CborCoseKeyHelpers
    {
        ...
        private static (ECParameters, HashAlgorithmName?) ReadECParametersAsCosePublicKey(CborReader reader)
        {
            ...
            static bool IsValidKtyCrvCombination(CoseKeyType kty, CoseCrvId crv)
            {
                ...
                {
                    (CoseKeyType.EC2, CoseCrvId.P256) => true,
                    (CoseKeyType.EC2, CoseCrvId.P384) => true,
                    (CoseKeyType.EC2, CoseCrvId.P521) => true,
                    (CoseKeyType.OKP, CoseCrvId.X255519) => true,
                    (CoseKeyType.OKP, CoseCrvId.X448) => true,
                    (CoseKeyType.OKP, CoseCrvId.Ed25519) => true,
                    (CoseKeyType.OKP, CoseCrvId.Ed448) => true,
                    _ => false,
                };
                ;
            }
            ...
            {
                if (crv == CoseCrvId.P256)
                    return ECCurve.NamedCurves.nistP256;
                if (crv == CoseCrvId.P384)
                    return ECCurve.NamedCurves.nistP384;
                if (crv == CoseCrvId.P521)
                    return ECCurve.NamedCurves.nistP521;
                if (crv == CoseCrvId.X255519 || crv == CoseCrvId.X448 || crv == CoseCrvId.Ed25519 || crv == CoseCrvId.Ed448)
                    throw new NotImplementedException("OKP type curves not implemented.");
                throw new CborContentException("Unrecognized COSE crv value.");
            }
